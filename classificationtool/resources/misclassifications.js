...
d3.json("data/schema.json", function(error, loadedSchema){	
	schema = loadedSchema;
/*					 
	var allLabelIds = labelIds(schema);	
	//labelIds = allLabelIds;
	var labels = d3.select("#labels").selectAll('input').data(allLabelIds);  	
...
var loadAndHandleData = function(){
	d3.json("data/misclassifications.json", function(error, segments) {
		...
		// get the unique label ids from the schema
		labels = labelIds(schema);
